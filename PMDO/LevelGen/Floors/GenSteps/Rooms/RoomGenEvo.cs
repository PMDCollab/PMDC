...
namespace PMDO.LevelGen
{
    ...
    public class RoomGenEvo<T> : RoomGen<T> where T : ITiledGenContext, IPostProcGenContext, IPlaceableGenContext<EffectTile>
    {
        ...
        public override void DrawOnMap(T map)
        {
            ...
            map.PostProcGrid[Draw.X + 1][Draw.Y + ROOM_OFFSET].Status[(int)PostProcType.Terrain] = true;
            map.SetTile(new Loc(Draw.X + 5, Draw.Y + ROOM_OFFSET), map.WallTerrain.Copy());
            map.PostProcGrid[Draw.X + 5][Draw.Y + ROOM_OFFSET].Status[(int)PostProcType.Terrain] = true;
