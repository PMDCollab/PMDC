...
class Localization:
    ...
    def open_sheet(self, credentials, id):
        ...
        self._id = id
        self.changelogger = open("../temp/changelog.txt", 'w', encoding='utf-8')
    ...
    def merge_data_text(self, txt_name, sheet_name, type_name, support):
        ...
        table = self._query_txt(os.path.join("..", "temp", txt_name+".txt"))
        # merge the data together, back into the google sheets
        self._merge_sheet_table(sheet_name, sheet, table)
        # read the google sheets again
        header, sheet = self._query_sheet(sheet_name)
        # write to output txt
        self._write_txt(os.path.join("..", "temp", txt_name + ".out.txt"), header, sheet)
        # update support
        self._update_support(header, sheet, support)
    ...
    def merge_enum_text(self, sheet_name, type_name, support):
        ...
        header, sheet = self._query_sheet(sheet_name)
        # load data from translation table file
        table = self._query_txt(os.path.join("..", "temp", type_name+".txt"))
        # merge the data together, back into the google sheets
        self._merge_sheet_table(sheet_name, sheet, table)
        # read the google sheets again
        header, sheet = self._query_sheet(sheet_name)
        # write to output txt
        self._write_txt(os.path.join("..", "temp", type_name + ".out.txt"), header, sheet)
        # update support
        self._update_support(header, sheet, support)
    ...
    def load_special_text(self, sheet_name, support):
        ...
        self._write_txt(os.path.join("..", "temp", "Special.out.txt"), header_row, sheet[1:6])

        self._write_txt(os.path.join("..", "temp", "Skin.out.txt"), header_row, sheet[6:8])

        self._write_txt(os.path.join("..", "temp", "Element.out.txt"), header_row, sheet[8:27])

        self._write_txt(os.path.join("..", "temp", "Rank.out.txt"), header_row, sheet[27:38])

        self._write_txt(os.path.join("..", "temp", "AI.out.txt"), header_row, sheet[38:])
