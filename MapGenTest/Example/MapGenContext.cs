...
using System.Linq;
using RogueElements;
using PMDOrigins.LevelGen;

namespace MapGenTest
{

    public class MapGenContext : IUnbreakableGenContext, IRoomGridGenContext, IViewPlaceableGenContext<StairsUp>, IViewPlaceableGenContext<StairsDown>, IPlaceableGenContext<Item>
    {
        ...
        public ITile UnbreakableTerrain { get { return new Tile(Map.UNBREAKABLE_TERRAIN_ID); } }

        public ITile GetTile(Loc loc) { return Map.Tiles[loc.X][loc.Y]; }
        public bool CanSetTile(Loc loc, ITile tile)
        {
            if (Map.Tiles[loc.X][loc.Y].ID == Map.UNBREAKABLE_TERRAIN_ID)
                return false;
